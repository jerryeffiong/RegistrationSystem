Module BackupRestoreModule
    ...
    Public Function SaveDB(ByVal FileName As String) As Boolean
        Try
            ...
            Dim Tmp As New RegistrationDBDataSetTableAdapters.MechanicalTableAdapter
            ...
            ' save the tables
            If Not SaveTableToDisk(Conn, Trans, "Civil", FS) Then
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not SaveTableToDisk(Conn, Trans, "Computer", FS) Then
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not SaveTableToDisk(Conn, Trans, "Electrical", FS) Then
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not SaveTableToDisk(Conn, Trans, "Geomatic", FS) Then
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not SaveTableToDisk(Conn, Trans, "Mechanical", FS) Then
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not SaveTableToDisk(Conn, Trans, "Construction", FS) Then
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not SaveTableToDisk(Conn, Trans, "ExecutiveTable", FS) Then
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not SaveTableToDisk(Conn, Trans, "Admins", FS) Then
                Conn.Close()
                FS.Close()
                Return False
            End If
    ...
    Public Function LoadDB(ByVal FileName As String) As Boolean
        ...
        Try
            ...
            ' clear all tables
            Dim CMD As New OleDb.OleDbCommand
            CMD.CommandText = " delete from Admins"
            CMD.Connection = Conn
            CMD.Transaction = Trans
            CMD.ExecuteNonQuery()
            CMD.Dispose()
            ...
            CMD = New OleDb.OleDbCommand
            CMD.CommandText = " delete from ExecutiveTable"
            CMD.Connection = Conn
            CMD.Transaction = Trans
            CMD.ExecuteNonQuery()
            CMD.Dispose()
            ...
            CMD = New OleDb.OleDbCommand
            CMD.CommandText = " delete from Construction"
            CMD.Connection = Conn
            CMD.Transaction = Trans
            CMD.ExecuteNonQuery()
            CMD.Dispose()
            ...
            CMD = New OleDb.OleDbCommand
            CMD.CommandText = " delete from Mechanical"
            CMD.Connection = Conn
            CMD.Transaction = Trans
            CMD.ExecuteNonQuery()
            CMD.Dispose()
            ...
            CMD = New OleDb.OleDbCommand
            CMD.CommandText = " delete from Geomatic"
            CMD.Connection = Conn
            CMD.Transaction = Trans
            CMD.ExecuteNonQuery()
            CMD.Dispose()
            ...
            CMD = New OleDb.OleDbCommand
            CMD.CommandText = " delete from Electrical"
            CMD.Connection = Conn
            CMD.Transaction = Trans
            CMD.ExecuteNonQuery()
            CMD.Dispose()
            ...
            CMD = New OleDb.OleDbCommand
            CMD.CommandText = " delete from Civil"
            CMD.Connection = Conn
            CMD.Transaction = Trans
            CMD.ExecuteNonQuery()
            CMD.Dispose()
            ...
            ' load the tables
            If Not LoadTableFromBackup(Conn, Trans, FS, "Civil") Then
                Trans.Rollback()
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not LoadTableFromBackup(Conn, Trans, FS, "Computer") Then
                Trans.Rollback()
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not LoadTableFromBackup(Conn, Trans, FS, "Electrical") Then
                Trans.Rollback()
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not LoadTableFromBackup(Conn, Trans, FS, "Geomatic") Then
                Trans.Rollback()
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not LoadTableFromBackup(Conn, Trans, FS, "Mechanical") Then
                Trans.Rollback()
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not LoadTableFromBackup(Conn, Trans, FS, "Construction") Then
                Trans.Rollback()
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not LoadTableFromBackup(Conn, Trans, FS, "ExecutiveTable") Then
                Trans.Rollback()
                Conn.Close()
                FS.Close()
                Return False
            End If
            If Not LoadTableFromBackup(Conn, Trans, FS, "Admins") Then
                Trans.Rollback()
                Conn.Close()
                FS.Close()
                Return False
